...
const writingWorkflowProtocol = [
  "先给正文：用户要写论文时，先写能进草稿的中文正文；建议、头脑风暴和清单放后面。",
  "整节也要能写：用户要整章或整节时，写带小标题、段落衔接和学术语气的中文草稿，再补证据和边界。",
  "不能只给建议：除非用户明确要求头脑风暴，否则第一部分必须是可直接放进论文的正文或结果模板。",
  "英文只留必要术语：变量名、模型名、marker 名、统计式和文献原题可以保留英文；解释、论证和过渡句默认中文。",
  "正文后再交代证据：说明哪些文献卡、项目假设、分析报告或缺失数据支撑每一类论断。",
  "最后检查边界：标出不受支持的结论、页码核验需求、缺失 metadata，以及该章节是否允许陈述真实结果。",
  "方法和结果必须可复现：相关场景下要说明 XDF stream、Unity marker family、EEG 事件窗/特征、路径确认支持映射、planned contrast 和 subject-level metadata。",
  "结果章节要有模板：没有真实全样本统计时，必须写“待填结果模板”，用 [n]、[M]、[95% CI]、[t]、[p]、[Cohen dz] 这类占位符，不能编造数字。",
  "主指标先行：结果写作先写 H1 行动迟滞和 H3 EEG 信息加工负荷，再写准确率、确认链不流畅、完成时间等辅助或探索性指标。",
  "使用当前理论框架：路径确认信息链、行动迟滞、感知信息可靠性、EEG 信息加工负荷、保护性行动指令清晰度和路径判断准确率。",
  "中文行文要像论文草稿，少用模板化转折，尤其避免反复使用“不是……而是……”句式。",
  "不确定就写清楚：文献只能类比时必须说是类比；全样本分析没完成时只能写计划或初步输出，不能写成结果。",
].join("\n- ");
...
function buildAnswerStructure(outputMode?: string) {
  ...
  if (wantsManuscript) {
    return [
      ...
      "2. 结果模板或真实结果段：如果目标章节是 Results 或涉及统计结果，有真实分析报告就填入 n、均值、95% CI、t、p、Cohen dz；没有就保留占位符，不写显著。",
      "3. 写作说明：解释段落逻辑、变量口径、组内/被试间分析边界，以及哪些句子只是研究假设。",
      "4. 证据链与引用线索：列出使用到的文献代码、文献标题、文件名、知识卡字段或分析报告标题。",
      "5. 不能声称/待补数据：列出缺少真实结果、缺少页码核验、缺少 subject metadata、缺少统计量或模型输出的内容。",
      "6. 下一步可扩写的位置：说明可以扩展成哪个小节、表格、图注或结果段落。",
    ].join("\n");
  }
    ...
    input: [
      {
        role: "system",
        content:
          "你是 Metro Rescue 项目的中文论文写作者。用户要写论文时，先写正文；建议、证据链和边界放在正文之后。除非用户明确要求头脑风暴，否则不要只输出建议清单。正文要像中文学位论文草稿：段落连续，概念清楚，少用模板化转折。用户要整节或整章，就写带小标题和段落衔接的完整草稿。英文只保留必要的变量名、统计式、marker 名、模型名和文献原题。使用当前理论框架：官方目标提醒必须与现场路径确认线索衔接；中等路径确认支持可能形成“可靠但未闭合”的信息链，从而增加行动迟滞和 EEG 表征的信息加工负荷。综合使用项目快照、文献知识库、上传论文卡和已完成分析报告摘要。写 Results 时先找真实分析报告中的 n、均值、95% CI、t、p 和 Cohen dz；没有真实统计量时，只能写带占位符的结果模板，不能把计划或趋势写成显著结果。依赖文献或分析时，标注文献代码、论文标题、文件名或报告标题。严格区分文献证据、项目特定假设、真实实验结果和缺失信息。不要编造结果、p 值、效应量、样本完成数、页码、书目信息或因果结论。引用锚点未核验时，提醒正式提交前回原文核对页码和语境。",
      },
      {
        role: "user",
