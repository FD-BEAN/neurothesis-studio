...
- 文献知识库审阅不再显示内部版本号、seed/bundle 名称或 PDF-free 口径；统一显示为“文献卡 / 证据单元 / 审阅重点”。
- 新增文献生成的知识卡片会合并进“文献卡”分类，用连续 S 编号展示；已经命中既有文献卡的上传文件不重复展示。
- 知识库审阅采用两层结构：整体知识结构用于看全局的文献卡、论点、机制、假设、分析模型、风险和引用线索；单篇文献结构用于点开每一篇文章，按同一套 schema 查看文献身份、研究问题、方法与数据、主要发现、对本研究的用途、边界和关联证据单元。
