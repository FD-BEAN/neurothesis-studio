...
- 当前不是完整 90 名被试数据；P01-P02 和 P27-P90 尚未在本地探索集中出现。
...
不建议为了显著性把该机制指标事后改成唯一主指标。若要提高其地位，应在完整 90 名被试分析前明确写成 protocol amendment：H1 主行为指标仍保留，`prompt_to_first_confirmation_s` 作为预先固定的机制性主/次指标之一。

## 下一步建议

1. 继续收齐完整 90 名被试，当前 24 名只适合做中期探索。
